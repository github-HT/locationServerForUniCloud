...
exports.main = async (event, context) => {
	...
	// 判断云函数URl化的参数还是uni-app请求的参数
	let _event;
	if (event.body) {
		_event = JSON.parse(event.body)
		_event.uniIdToken = event.headers.authorization
	} else {
		_event = event
	}
	/*
	...
	*/
	...
	const deviceInfo = _event.deviceInfo || {};
	let params = _event.params || {};
	/*
	...
	*/
	...
	switch (action) { //根据action的值执行对应的操作
		...
		case 'getCurrentUserInfo':
			...
			break;
		case 'locationUpload':
			if (params.locationArr && params.locationArr.length > 0) {
				for (let s of params.locationArr) {
					db.collection('uni-id-locations').add({
						user_id: params.uid,
